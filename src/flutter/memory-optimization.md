...
3A Launcher 是一个 Flutter Windows 渲染器启动器。在线资源库和下载记录上线后，内存从原来的 200MB 一路涨到 400—700MB。这次没有靠一句“Flutter 桌面端本来就占内存”带过，而是顺着大文件、图片缓存、数据库查询和页面常驻逐项排查。
...
回头看，真正有效的改动都落在数据什么时候进内存、又在内存里停多久：
...
Release 包实测，所有界面打开后稳定在 **200MB 以下**。启动时会因为数据库初始化和本地文件校验短暂冲到 400MB，随后很快回落；相比优化前的 400—700MB，常驻内存下降了六成以上。
