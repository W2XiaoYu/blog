...
- [用 GORM 设计两步验证 TOTP](./database-design/totp-two-factor) — 动态验证码的原理与落库：共享密钥、时间片、防重放、备份码和登录岔口
- [用 GORM 把一级邀请分佣做清楚](./database-design/direct-invite-commission) — 从一枚邀请码开始，把支付、结算和退款的账走完整
- [用 GORM 设计商品 SPU 与 SKU](./database-design/product-spu-sku) — 商品和规格怎么分层，价格、库存和订单快照落在哪张表
