...
账号系统不必一开始就做成一座大平台，先守住几条不容易返工的边界：
...
这层骨架站稳以后，再接 QQ、微博、Apple 或企业登录，主要工作只是增加 provider 适配。订单、项目和会员仍然只认 `users.id`，不必跟着登录方式一起改。

账号合并也因此有路可循：先证明两个账号都属于当前用户，再处理冲突、搬迁数据，最后停用 source、撤销旧 Session，把过程完整地留在合并记录里。
