...
问题归根到底有两个：多种登录方式怎样认回同一个人，已经重复创建的账号又怎样合到一起。下面用 GORM v2 和 PostgreSQL 把这两件事从表结构一直写到合并事务。
...
所以账号合并要做的，就是把登录身份和允许迁移的业务数据，从 source 的 `user_id` 改到 target。不能改归属的订单或流水，就保留原来的 `user_id`，查询时再通过 `merged_into_user_id` 找到当前账号。
...
锁住以后的再查一遍不是多余的：用户停留在确认页面时，另一个请求可能又绑定了新手机号，也可能有人同时发起了另一场合并。页面上看到的检查结果，到真正执行时不一定还有效。
...
这些边界守住以后，再接 QQ、微博、Apple 或企业登录，主要工作只是增加 provider 适配，订单、项目和会员仍然只认 `users.id`，不必跟着登录方式一起改。

账号合并的流程也就固定下来了：先证明两个账号都属于当前用户，再处理冲突、搬迁数据，最后停用 source、撤销旧 Session，整个过程留在合并记录里。
