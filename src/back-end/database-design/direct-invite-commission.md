...
这里只讨论技术上的记账方式。实际运营还会涉及宣传方式、推广资格、税务和提现，正式上线前仍要按具体业务做合规审核。
...
计算时统一向下取整。商品实付 99.99 元，比例 10%，佣金是 999 分，也就是 9.99 元。每笔订单独立计算，不需要再做四舍五入后的补差。
...
这段 migration 要放在用户表和订单表之后。示例里的 `OrderID` 只保留了业务关联，没有建外键；如果订单只归档、不物理删除，可以再补一条外键。

开发阶段用 `AutoMigrate` 很省事。到了正式环境，表结构的变化还是写成带版本号的 migration，哪次上线加了什么、回滚时该撤什么，都会清楚很多。

## 邀请码只认第一次

邀请关系适合在注册后，或者第一次进入系统时绑定。重复提交同一个邀请码按成功处理；绑定成功后再换别的邀请码，直接拒绝。

只拦“自己邀请自己”还不够。假如 A 已经邀请了 B，又允许 A 填上 B 的邀请码，两个人就围成了一个环，谁买东西都能给另一个人返钱。

邀请码绑定并不是高频操作，这里干脆用 PostgreSQL 的事务级 advisory lock，让绑定请求依次通过。写入前沿着邀请链往上查一遍，既能挡住已有的环，也不会被两个同时到达的请求钻空子。
...
代码拿到 `relation.InviterID` 后没有再往上查，一级分佣就停在这里。
...
发起提现时，在事务里锁住账户，把钱从 `available_amount` 挪到 `frozen_amount`。打款成功后再计入累计提现，失败则退回可用余额，接口超时也不会留下一笔说不清的差额。
...
两边邀请关系发生冲突时，宁可留给人工处理，也别临时挑一个“注册更早”的账号覆盖过去。
...
整套设计没有团队、等级和多层比例，每张订单只查一次买家的直接邀请人。

把关系绑定、支付幂等、规则快照、延迟结算和退款冲正这几点守住，账就能对得上。以后加提现审核、税务信息或商家分摊，在现有表上扩展即可，不用改邀请关系。
