...
透明分层窗口里有一个 slider，用来调音量或进度。按下鼠标时光标还是 `ew-resize`，稍微拖出组件范围，它却变回箭头，有时甚至穿过透明区域落到桌面上。一个很小的交互问题，背后同时牵涉 DOM 命中和窗口穿透。

最后的处理没有依赖全局 CSS class：拖拽期间铺一层透明 DOM 接住光标，再暂时关闭窗口穿透。
...
DOM 覆盖层管的是光标跟随，窗口 API 管的是透明穿透，两边各做自己的事。这个办法不只适用于 slider，透明分层窗口里凡是需要在一段拖拽期间固定全局光标，都可以沿用同样的思路。
